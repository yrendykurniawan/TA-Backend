        ...
        for i in range(2, len(sentence)):
            ...
            for tagEntry in self.WH.tagProbs(sentence[i]):
                ...
                for t in tagMatrix[i - 2]:
                    ...
                    if self.debug:
                        print("BigramDecode : ", sentence[
                            i], "\nTag:", tagEntry, "\nnow:", prob, " prev:", t.probs, " ", bigramProb, " ", \
                        self.WH.tagProbs(sentence[i])[tagEntry])
...
class MainTagger:
    ...
    def __init__(self, fileLexicon, fileNGram, NGramType, maxAffixLength=3, Treshold=3, minWordFreq=0, modeAffixTree=0,
                 debug=False, LambdaBigram=0.2, TwoPhaseType=0, beamFactor=500.0, useLexicon=0):
    ...
    def loadData(self):
        ...
        if self.modeAffixTree != 3:
            self.ovp = OOVWordProb(self.model.getLexicon(), self.model.getUniGrams(), self.maxAffixLength,
                                   self.Treshold, self.minWordFreq, self.modeAffixTree, self.debug)

        # print "phase0 OK"

        self.wp = KnownWordProb(self.model.getLexicon(), self.model.getUniGrams(), self.model.getNumberTags(), self.ovp,
                                self.dl, self.debug)
        self.np = NGramProb(self.model.getUniGrams(), self.model.getBiGrams(), self.model.getTriGrams(),
                            self.model.getQuatoGrams(), self.LambdaBigram)
